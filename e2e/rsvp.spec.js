...
test("attending — fills full form with two extra guests and submits", async ({ page }) => {
  // Fill primary name
  ...
  await page.getByRole("button", { name: /Kid 🎉/ }).first().click();

  // Add second guest: Marco, adult, vegan
  await page.getByRole("button", { name: "+ Add another person" }).click();
  await page.getByPlaceholder("Full name").nth(1).fill("Marco");
  await page.getByRole("button", { name: /Adult 🥂/ }).last().click();
  ...
  await expect(page.getByText("We can't wait to see you in Sicily!")).toBeVisible();
});

// ─── Journey 2: declining ────────────────────────────────────────────────────

test("declining — no conditional fields shown, correct payload, decline screen", async ({ page }) => {
  ...
  await expect(page.getByText("How are you planning to get there?")).not.toBeVisible();
...
test("validation — blocks empty submission and shows errors", async ({ page }) => {
  await page.getByRole("button", { name: "Send" }).click();

  await expect(page.getByText("Please enter your name.")).toBeVisible();
  await expect(page.getByText("Please let us know if you'll be joining us.")).toBeVisible();
